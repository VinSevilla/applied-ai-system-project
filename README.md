...
The PawPal+ system has several limitations and potential risks mainly due to how it relies on a small, fixed knowledge base and AI-generated scheduling. Its care guidelines are based on general Western pet care norms, which may not apply to all pets, especially those with specific medical or lifestyle needs. The system also struggles with non-English inputs and makes assumptions about which tasks can be combined, which can lead to incorrect scheduling in edge cases. Additionally, because the AI does not distinguish between routine and critical tasks, users might mistakenly rely on it for medical decisions, which could be harmful. Testing revealed that the AI often ignored important rules like spacing between tasks, and its evaluator failed to catch many errors, showing that AI alone is not reliable for enforcing correctness. Overall, the most important takeaway is that the AI works best as a suggestion tool, while strict rules and validation should be handled through code to ensure accuracy and safety.
